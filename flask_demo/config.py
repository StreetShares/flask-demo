...
class Custom(object):
    ...
    configuration options for Flask, Celery, or their extensions
    go here.
    """
    ADMINS = ['me@me.test']
    ...
    _SQLALCHEMY_DATABASE_DATABASE = 'flask_demo'
    _SQLALCHEMY_DATABASE_HOSTNAME = 'localhost'
    _SQLALCHEMY_DATABASE_PASSWORD = ''
    _SQLALCHEMY_DATABASE_USERNAME = 'aahlers'

    PUBLIC_PORT = 5000
    PRIVATE_PORT = 5001


class CeleryConfig(Custom):
...
class DefaultConfig(CeleryConfig):
    ...
    SQLALCHEMY_DATABASE_URI = property(
        lambda self: 'mysql://{u}:{p}@{h}/{d}'.format(
            d=quote_plus(self._SQLALCHEMY_DATABASE_DATABASE),
            h=quote_plus(self._SQLALCHEMY_DATABASE_HOSTNAME),
